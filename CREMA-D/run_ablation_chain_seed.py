...
OUT_ROOT = f"/root/autodl-tmp/ablation_chain_ACE_seed{SEED}"
...
def cosine_lr_factor(epoch_idx_1based: int, total_epochs: int, warmup_epochs: int) -> float:
    """
    LR multiplier in [0,1]. Linear warmup then cosine decay.
    epoch_idx_1based: 1..total_epochs
    """
    e = epoch_idx_1based
...
class ExtraTransformerEncoder(nn.Module):
    ...
    def forward(self, x, key_padding_mask=None):
        # key_padding_mask: (B, T) with True for pads
...
class E2EModel(nn.Module):
    ...
    def forward(self, input_values, attention_mask, x_trad, alpha: float):
        ...
        if self.extra_trf is not None:
            # IMPORTANT FIX:
            # Transformer expects key_padding_mask shape (B, T), where T matches hs length.
            # Our attention_mask is waveform-level (B, L=48000), so it's not compatible.
            # Since we pad/trim all waveforms to fixed length, there is no padding at hidden level.
            B, T, _ = hs.shape
            key_padding_mask = torch.zeros((B, T), dtype=torch.bool, device=hs.device)  # no pads
...
def make_class_balanced_weights(y_train: np.ndarray, num_classes: int) -> torch.Tensor:
    """
    Simple inverse-frequency weights: w_c = N / (K * n_c), normalized to mean=1
    """
...
# =========================
# Main: build split once, run A~F (chain ablation)
# =========================
def main():
    seed_all(SEED)
    ...
    # speaker-independent split (fixed)
    ...
    # ===== Chain Ablation A→F (step-by-step add modules) =====
    # A: base
    # B: + extraTRF (1 layer)
    # C: + hidden specaug
    # D: + class-balanced
    # E: + SAT ramp
    # F: + deepen extraTRF to 2 layers (final upgrade step so F != E)
    exps = [
        # A: base
        ExpConfig(
            name="A_base",
        ...
        # C: A + extraTRF(1) + specaug  （我们直接跳过 B，但配置等价于链式走到 C）
        ExpConfig(
            name="C_plus_specaug",
            unfreeze_last_n=2,
            use_extra_trf=True,
        ...
        # E: C + class-balanced + SAT ramp （等价于链式走到 E）
        ExpConfig(
            name="E_plus_SATramp",
            unfreeze_last_n=2,
            use_extra_trf=True,
    ...
    for cfg in exps:
        ...
        # write progressively (so if later exp crashes, you still have previous results)
