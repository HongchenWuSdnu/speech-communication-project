# =========================
# train_stage2_last2_satweak.py
# Stage-2 exploration: unfreeze last2 + weak SAT (optional warmup)
...
class CollatorFixedLen:
    def __call__(self, batch):
        ...
        attn_mask = torch.ones_like(waveforms, dtype=torch.long)
...
# =========================
# Model (last2 unfreeze + SAT weak)
# =========================
class E2E_Net_Last2_SAT(nn.Module):
    def __init__(self, num_emotions: int, num_speakers: int, train_last_n_layers: int = 2, speaker_head_dim: int = 32):
        super().__init__()
        ...
        # unfreeze last N transformer layers (wav2vec2-base: 12 layers)
        last_layers = list(range(12 - train_last_n_layers, 12))
    ...
    def forward(self, input_values, attention_mask, x_trad, alpha: float):
        hs = self.wav2vec2(input_values=input_values, attention_mask=attention_mask).last_hidden_state

        lstm_out, _ = self.lstm(hs)
        attn = F.softmax(self.temporal_attn(lstm_out), dim=1)
        h_deep = torch.sum(lstm_out * attn, dim=1)

        h_trad = self.trad_stream(x_trad)
...
def train_one_epoch(model, loader, optimizer, device, criterion_emo, criterion_spk,
                    alpha, lambda_spk, accum_steps=1, max_grad_norm=1.0):
    model.train()
...
if __name__ == "__main__":
    ...
    NUM_EPOCHS   = 40
    BATCH_SIZE   = 16
    ACCUM_STEPS  = 2          # effective 32
    NUM_WORKERS  = 4

    # Unfreeze + SAT weak config
    TRAIN_LAST_N_LAYERS = 2

    WARMUP_EPOCHS = 8         # longer warmup
    ALPHA_SAT     = 0.03      # weak GRL
    LAMBDA_SPK    = 0.005     # weak spk loss

    LR_W2V       = 5e-6       # smaller than A1/A2
    LR_HEAD      = 5e-4
    WEIGHT_DECAY = 1e-4
    ...
    model = E2E_Net_Last2_SAT(
        num_emotions=len(emo_enc.classes_),
        num_speakers=len(spk_enc.classes_),
        train_last_n_layers=TRAIN_LAST_N_LAYERS,
        speaker_head_dim=32
    ).to(device)
    print(f"Trainable params: {count_trainable_params(model)/1e6:.2f}M (last{TRAIN_LAST_N_LAYERS} unfrozen)")
    ...
    optimizer = optim.AdamW(
        [
            {"params": wav2vec2_params, "lr": LR_W2V},
    ...
    best_uar = -1.0
    best_path = os.path.join(OUT_DIR, "E2E_stage2_last2_satweak_best.pth")

    print("\n⚔️ Start training (Stage-2: last2 + weak SAT)...")
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        print(f"Epoch {epoch:02d} | Train Emo={tr_emo:.4f} Spk={tr_spk:.4f} | "
              f"Val Acc={acc:.2f}% UAR={uar:.2f}% MF1={mf1:.2f}% | SpkAcc={spk_acc:.2f}% | "
              f"alpha={alpha:.3f} lam={lam:.3f}")
